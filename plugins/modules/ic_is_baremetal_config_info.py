...
short_description: Retrieve initial configuration used by a BMS on IBM Cloud.
...
EXAMPLES = r'''
- name: Retrieve BMS configuration
  ic_is_baremetal_config_info:
...
def run_module():
    ...
    result = vsi_instance.get_server_configuration(instance)
    if "errors" in result:
